...
const OrdersList = () => {
    ...
    const setOrders = () => {
        ...
        orders.forEach(order => {
            data.rows.push({
                id: order._id,
                numofItmes: order.orderItems.length,
                ...
                actions: <Fragment>
                    <Link to={`/admin/order/${order._id}`} className="btn btn-primary py-1 px-2"><i className="fa fa-eye"></i></Link> <button className="btn btn-danger py-1 px-2 ml-2"><i className="fa fa-trash"></i></button>
                </Fragment>
